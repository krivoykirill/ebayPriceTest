...
def mergeItems(ebayResponse,queryData):
    totalAdded=0
    #add items with end_time after the last date in DB records. Otherwise, add all items
    try:
        latest_date=datetime.datetime.strptime(queryData[0]['end_time'],'%Y-%m-%d %H:%M:%S')
    except IndexError as e:
        latest_date=datetime.datetime(1970,1,1)
    for i in reversed(range(len(ebayResponse))):
        if (datetime.datetime.strptime(ebayResponse[i]['end_time'],'%Y-%m-%d %H:%M:%S')  > latest_date):
            queryData.insert(0,ebayResponse[i])
            totalAdded+=1
    return queryData,totalAdded
...
if (len(finalItems)<=0):
    ...
    cur2=con.cursor()
    #array from ebay as a dict and then pandas series
    if (queryDataId!=None):
        cur2.execute('select * from query_data where id={}'.format(queryDataId))
        items=cur2.fetchone()[2]['items']
    else:
        items=[]
    
    
    finalItems,totalAdded=mergeItems(finalItems,items)
    print('total new records added: '+str(totalAdded))

    #mining data, setting up pandas
    df=pd.read_json(json.dumps(finalItems))
    ...
    try:
        #deleting previous query_data info related to the refreshed user query
        cur.execute("delete from query_data where query_id={}".format(id))
